...
const mp3Content = ref([]);
const mp4Content = ref([]);
...
function scanDefaultDirectory() {
  fetch(`${baseUrl}/api/services/get-default-assets-content`)
    ...
    .then((data) => {
      ...
      mp4Content.value = data.contents.filter((filename) =>
        filename.endsWith(".mp4")
      );
      mp3Content.value = data.contents.filter((filename) =>
...
const mp4Models = computed(() => {
  return mp4Content.value.map(
    (filename) => `${baseUrl}/public/twin/assets/${filename}`
  );
});

const mp3Models = computed(() => {
  return mp3Content.value.map(
    (filename) => `${baseUrl}/public/twin/assets/${filename}`
  );
});
...
<template>
  ...
  <div id="images">
    ...
    <div class="d-flex justify-content-start flex-wrap container container-cus">
      <div v-for="(src, index) in jpgModels" :key="index" class="div-model">
        <img class="model-views" camera-controls alt="Model" :src="src"></img>
        <div class="content-div d-flex justify-content-between">
          <p>{{ truncateFilename(src) }}</p>
          <button class="copy-btn" @click="copyFilename(src)"><i class="fa-solid fa-copy fa-lg"
              style="background-color: rgba(240, 248, 255, 0);"></i></button>
  ...
  <div id="videos" style="padding-bottom: 50px;">
    ...
    <div class="d-flex justify-content-start flex-wrap container container-cus">
      <div v-for="(src, index) in mp4Models" :key="index" class="div-model">
        <video class="model-views" controls :src="src" style="object-fit: cover; transform: translateY(5px)"></video>
        <div class="content-div d-flex justify-content-between">
          <p>{{ truncateFilename(src) }}</p>
          <button class="copy-btn" @click="copyFilename(src)"><i class="fa-solid fa-copy fa-lg"
              style="background-color: rgba(240, 248, 255, 0);"></i></button>
  ...
  <!-- End videos -->
  <div id="audio" style="padding-bottom: 50px;">
    <div class="container container-text">
      <h1 class="heading">Audio</h1>
    </div>
    <div class="d-flex justify-content-start flex-wrap container container-cus">
      <div v-for="(src, index) in mp3Models" :key="index" class="div-model">
        <audio class="audio-views" controls :src="src"></audio>
        <div class="content-div d-flex justify-content-between">
          <p>{{ truncateFilename(src) }}</p>
          <button class="copy-btn" @click="copyFilename(src)"><i class="fa-solid fa-copy fa-lg"
              style="background-color: rgba(240, 248, 255, 0);"></i></button>
