...
const labelValues = ref(['']); // Array to store values of label inputs

// Watch for changes in labelValues and update receivedFocusedNodeData accordingly
watch(labelValues, (newLabelValues) => {
  console.log(newLabelValues);
});
...
function addInput() {
  labelValues.value.push(''); // Add an empty string for the new label input
}
</script>
        ...
        <div class="row justify-content-between text-left">
          <div class="input-row gap-2 d-flex justify-content-between">
            <div class="input-row-col-2">
              <label class="form-control-label px-3" for="modalName" style="width: auto;">Component Name</label>
              <input type="text" id="modalName" v-model="receivedFocusedNodeData.type" />
            </div>
            <div class="input-row-col-2">
              <label class="form-control-label px-3" for="modalName" style="width: auto;">Category</label>
              <input type="text" id="modalName" v-model="receivedFocusedNodeData.category" />
            </div>
          </div>
          <div class="input-row gap-2 d-flex justify-content-between">
            <div class="input-row-col-2">
              <label class="form-control-label px-3" for="modalName" style="width: auto;">Node Name</label>
              <input type="text" id="modalName" v-model="receivedFocusedNodeData.properties.name" />
            </div>
            <div class="input-row-col-2">
            </div>
          </div>
          <div class="row" style="padding: 0; margin: 0;" v-for="(value, index) in labelValues" :key="index">
            {{ console.log("index", index) }}
            <div class="input-row gap-2 d-flex justify-content-between">
              <div class="input-row-col-2">
                <label class="col-md-12 form-control-label px-3 input-label" :for="'label' + index">Property
                  Label</label>
                <input :id="'label' + index" class="input-box" type="text" v-model="labelValues[index]" />
              </div>
              <div class="input-row-col-2">
                <label class="col-md-12 form-control-label px-3 input-label" :for="'value' + index">Property
                  Value</label>
                <input :id="'value' + index" class="input-box" type="text"
                  v-model="receivedFocusedNodeData.properties[labelValues[index]]" />
              </div>
            </div>
          </div>
          <div class="input-row">
            <input type="button" class="w-25 btn btn-success ms-5" value="Add New Property" @click="addInput" />
          </div>
        </div>
